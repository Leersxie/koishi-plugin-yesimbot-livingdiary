...
export interface Config {
  ...
  /** 是否启用手动触发日记：在会话中输入关键词即可立即执行每日日记流程 */
  diaryManualEnabled: boolean
  /** 手动触发日记的关键词，消息完全匹配时触发 */
  diaryManualKeyword: string
...
export const Config = Schema.object({
  ...
  diaryManualEnabled: Schema.boolean()
    .default(false)
    .description('是否启用手动触发日记：在会话中输入关键词即可立即执行每日日记流程（不受 diaryTime 时间限制）'),
  diaryManualKeyword: Schema.string()
    .default('')
    .description('手动触发日记的关键词，消息内容完全匹配时触发一次日记生成并发布（须同时开启 diaryManualEnabled）'),
